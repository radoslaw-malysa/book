<?php
   ...
   #
   # ========================================================================#
namespace App\Model\Support\Magician;
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/ 
  ...
  #       forceStretch can be applied to options 1,2,3 and 4  
    ...
    //korekta by Radek bo zle cropowal pielony magik
    $optimalWidth  = ceil($dimensionsArray['optimalWidth']);
    $optimalHeight = ceil($dimensionsArray['optimalHeight']);
    //oraz spieprzone bylo pozycjonowanie watermarka
    $this->width_resized = $optimalWidth;
    $this->height_resized = $optimalHeight;
  ...
  #
  {
    ...
    if (strstr($pos, 'x')) {
      ...
      switch ($pos) {
        ...
        case 'tr':
          $width = $this->width_resized - $assetWidth - $padding; 
          $height = 0 + $padding;
          break;
...
## -------------------------------------------------------- 
  ...
  #       third party class to save as bmp.
    ...
    //tuatj na php8 error jakowys jest radek
...
## --------------------------------------------------------
    ...
    $type = gettype($this->imageResized);
    if ($type !== 'boolean' && $type !== 'bool' && $type !== 'NULL' && $type !== 'unknown type') {
      imagedestroy($this->imageResized);
    }
  }
